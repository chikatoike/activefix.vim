" this file is based on syntastic/syntax_checkers/ocaml.vim
"============================================================================
"File:        ocaml.vim
...
function! activefix#syntastic#ocaml#config()
...
function s:GetOcamlcMakeprg()
    if g:syntastic_ocaml_use_janestreet_core
        ...
        let build_cmd .= activefix#syntastic#expand_target('ocaml', g:syntastic_ocaml_janestreet_core_dir)
        let build_cmd .= " -c ".activefix#syntastic#expand_target('ocaml', '%')
        return build_cmd
    else
        return "ocamlc -c ". activefix#syntastic#expand_target('ocaml', '%')
    endif
endfunction

function s:GetOcamlBuildMakeprg()
    return "ocamlbuild -quiet -no-log -tag annot,". s:ocamlpp. " -no-links -no-hygiene -no-sanitize ".
                \ shellescape(activefix#syntastic#expand_target('ocaml', '%:r')).".cmi"
endfunction
...
function s:GetOtherMakeprg()
    ...
    let extension = activefix#syntastic#expand_target('ocaml', '%:e')
    let makeprg = ""

    if match(extension, 'mly') >= 0 && executable("menhir")
        " ocamlyacc output can't be redirected, so use menhir
        let makeprg = "menhir --only-preprocess ".shellescape(activefix#syntastic#expand_target('ocaml', '%')) . " >/dev/null"
    elseif match(extension,'mll') >= 0 && executable("ocamllex")
        let makeprg = "ocamllex -q -o /dev/null ".shellescape(activefix#syntastic#expand_target('ocaml', '%'))
    else
        let makeprg = "camlp4o -o /dev/null ".shellescape(activefix#syntastic#expand_target('ocaml', '%'))
    endif
