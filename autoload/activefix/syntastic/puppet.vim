" this file is based on syntastic/syntax_checkers/puppet.vim
"============================================================================
"File:        puppet.vim
...
function! s:getPuppetLintErrors()
    ...
    let makeprg = 'puppet-lint --log-format "\%{KIND} [\%{check}] \%{message} at \%{fullpath}:\%{linenumber}" '.g:syntastic_puppet_lint_arguments.shellescape(activefix#syntastic#expand_target('puppet', '%'))
    let errorformat = '%t%*[a-zA-Z] %m at %f:%l'
    return { 'makeprg': makeprg, 'errorformat': errorformat, 'subtype': 'Style' }
endfunction
...
function! s:getPuppetMakeprg()
    ...
    if s:IsVersionAtLeast(s:PuppetVersion(), [2,7,0])
        let makeprg = 'puppet parser validate ' .
                    \ shellescape(activefix#syntastic#expand_target('puppet', '%')) .
                    \ ' --color=false'
        ...
        let makeprg = 'puppet --color=false --parseonly --ignoreimport '.shellescape(activefix#syntastic#expand_target('puppet', '%'))
    endif
    return makeprg
endfunction

function! activefix#syntastic#puppet#config()
    let errors = []
